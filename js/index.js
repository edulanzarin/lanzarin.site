...
import { getDatabase, ref, set, get, onValue, push } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-database.js';
        ...
        if (snapshot.exists()) {
            ...
            snapshot.forEach(childSnapshot => {
                ...
                if (idUsuarioLogado === ADMIN_ID || userId === ADMIN_ID) {
                    ...
                    if (userId !== idUsuarioLogado) { // Exclui o próprio usuário logado da lista
...
function carregarMensagens(chatId) {
    ...
    const chatPath = idUsuarioLogado < chatId ? `${idUsuarioLogado}_${chatId}` : `${chatId}_${idUsuarioLogado}`;
...
function mostrarMensagens(mensagens, idUsuarioLogado) {
    ...
    mensagens.forEach(mensagemData => {
        ...
        messageElement.className = `message ${mensagemData.id_usuario1 === idUsuarioLogado ? 'sent' : 'received'}`;
        
        // Descriptografa a mensagem
        const secretKey = 'sua-chave-secreta'; // Mesma chave usada para criptografia
        const decryptedMessage = CryptoJS.AES.decrypt(mensagemData.texto, secretKey).toString(CryptoJS.enc.Utf8);
...
function enviarMensagem(message) {
    ...
    const idUsuarioLogado = sessionStorage.getItem('id_usuario');
    const chatPath = idUsuarioLogado < chatIdAtual ? `${idUsuarioLogado}_${chatIdAtual}` : `${chatIdAtual}_${idUsuarioLogado}`;
...
function mostrarConfirmacao(tipo, callback) {
    ...
    modalBody.textContent = tipo === 'delete' ? 'Você tem certeza que deseja apagar todas as mensagens deste chat?' : ''; // Remove a mensagem de sucesso
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        chatList.addEventListener('click', (event) => {
            if (event.target.classList.contains('chat-item')) {
                ...
                document.querySelector('.chat-header').textContent = event.target.textContent; // Apenas o nome do usuário
